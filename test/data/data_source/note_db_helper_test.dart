...
void main() {
  test('db test', () async {
    ...
    await noteDbHelper.insertNote(note: const Note(title: 'test', content: 'test', color: 1, timestamp: 1));

    expect((await noteDbHelper.getNotes()).length, 1);

    Note note = (await noteDbHelper.getNoteById(id: 1))!;
    expect(note.id, 1);

    await noteDbHelper.updateNote(note: note.copyWith(
      title: 'change'
    ));

    note = (await noteDbHelper.getNoteById(id: 1))!;
    expect(note.title, 'change');

    await noteDbHelper.deleteNote(note: note);
